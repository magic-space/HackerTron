...
skidinc.server.getEffectsToStr = function(what) {
    ...
    for (var effect in server.effects) {
        ...
        str += '<b>+' + fix(effect) + '</b> zu ' + i + ' multiplier';
...
skidinc.server.list = function() {
    ...
    for (var i = 0; i < skidinc.server.servers.length; i++) {
        ...
        str += '<b>*</b> <z>' + server.id + '</z>: Kosten <b>$' + fix(cost) + '</b>, ' + effects + '.<br>';
    };
...
skidinc.server.buy = function(item, amount) {
    if (skidinc.server.servers.indexOf(item) == -1)
        return skidinc.console.print('<x>ERR</x> Server Typ<b>' + item + '</b> existiert nicht.');
    ...
    if (typeof amount == 'number' && amount == 1) {
        ...
        if (skidinc.player.money >= cost && skidinc.server.owned[server.index] + 1 <= server.max) {
            ...
            return skidinc.console.print('<z>SERVER</z> Du hast erfolgreich deinen <b>' + server.id + '</b> Server aktualisiert.');
        };
        
        if (skidinc.server.owned[server.index] + 1 > server.max)
            return skidinc.console.print('<x>ERR</x> Dieser Server hat ein maximal Level, dies liegt bei <b>' + server.max + '</b>.');
